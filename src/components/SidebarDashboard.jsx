...
const SidebarDashboard = () => {
        ...
        const onTemplateSelect = (e) => {
                if (e.files.length > 1) {
                        toast.current.show({ severity: 'warn', summary: 'Peringatan', detail: 'Hanya boleh satu file' });
        ...
        const onTemplateUpload = async (e) => {
                ...
                try {  
                        ...
                        toast.current.show({ severity: "error", summary: "Gagal upload gambar", detail: err.message})
                }
        }
        ...
        const headerTemplate = (options) => {
                ...
                const formatedValue = fileUploadRef && fileUploadRef.current ? fileUploadRef.current.formatSize(totalSizeImage) : '0 B';
                ...
                return (
                        <div className={ className } style={{ backgroundColor: 'transparent', display: 'flex', alignItems: 'center' }}>
                                ...
                                <div className="flex-item-center gap-3 sm:ml-auto">
                                        <span className='dark:!text-gray-500 !mb-1'>{formatedValue} / 1 MB</span>
                ...
                return (
                        <div className="flex items-center gap-10 flex-wrap dark:!text-gray-500">
                                <div className="flex items-center">
                                        <img src={file.objectURL} role='presentation' className='w-[60px] xs:w-[100px]' alt={file.name} />
                                        <span className="flex flex-col text-start ml-3 line-clamp-1">
                                                {file.name}
                                                <small>{new Date().toLocaleDateString()}</small>
                                        </span>
                                </div>
                                <Tag value={props.formatSize} severity="warning" className='!px-3 !py-2 !hidden sm:!block' />
                                <Button
                                        icon="pi pi-times"
        ...
        useEffect(() => {
                ( async() => {
                        ...
                        const genreListObject =  data_genre.genre.map((item) => ({nama: item, value: item}))
                        const statusListObject =  data_status.status.map((item) => ({nama: item, value: item}))
        ...
        const handleSave = () => {
                toast.current.show({severity: "warn", summary: "Tidak dapat melakukan tindakan ini", detail: "Data sedang dalam pemeliharaan by dev asi-web"})
        }
        ...
        const bodyDashboard = () => {
                return (
                        <div className='mt-5'>
                                <span className="p-4 text-2xl font-extrabold text-gray-300 dark:text-gray-500">Dashboard</span>
                                <div className="p-4 grid h-fit w-fit grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                                                <div className="flex flex-col items-center gap-4 border dark:border-gray-700 border-gray-300 rounded-md p-5 xs:p-10 h-fit w-full sm:w-[300px]">
                                                        <div className="flex gap-6 items-center">
                                                                ...
                                                                <div className="text-center">
                                                                        <p className="dark:text-gray-200 text-[.6rem] xs:text-base">Jumlah Anime</p>
                                                                        <p className="text-gray-300 dark:text-gray-500 text-2xl xs:text-4xl font-extrabold">{animeList.length}</p>
                                                                </div>
                                                        </div>
                                                ...
                                                <div className="flex flex-col items-center gap-4 border dark:border-gray-700 border-gray-300 rounded-md p-5 xs:p-10 h-fit w-full  sm:w-[300px]">
                                                        <div className="flex gap-6 items-center">
                                                                ...
                                                                <div className="text-center">
                                                                        <p className="dark:text-gray-200 text-[.6rem] xs:text-base">Jumlah Admin</p>
                                                                        <p className="text-gray-300 dark:text-gray-500 text-2xl xs:text-4xl font-extrabold">{adminHomeList.length}</p>
                                                                </div>
                                                        </div>
                                                </div>
                                </div>
        ...
        const bodyAnime = () => {
                return (
                        <BlockUI className="!items-start dark:!bg-gray-900/90 !pt-80" blocked={true} template={<i className="text-gray-600 pi pi-lock !text-[5rem]"></i>}>
                                <div className='mt-5 overflow-y-auto max-h-[80vh] '>
                                        <span className="p-4 text-2xl font-extrabold text-gray-300 dark:text-gray-500">Tambah Anime</span>
                                        <div className="p-4">
                                                ...
                                                <div className="p-inputgroup">
                                                        ...
                                                        <InputText placeholder="Judul Anime" className='dark:!text-gray-200 dark:!bg-gray-700/60'/>
                                                </div>
                                                ...
                                                <div className='mt-5'>
                                                        ...
                                                        <FileUpload  ref={fileUploadRef} name="demo[]" customUpload multiple={false} accept="image/*" maxFileSize={1000000}
                                                                uploadHandler={onTemplateUpload} onSelect={onTemplateSelect} onError={onTemplateClear} onClear={onTemplateClear}
                                                                headerTemplate={headerTemplate} itemTemplate={uploadItemTemplate} emptyTemplate={emptyTemplate}
                                                ...
                                                <div className="flex items-center gap-2 mb-3">
                                                        <Rating value={rating} onChange={(e) => setRating(e.target.value)} cancel={true}/> 
                                                        <span>{rating}/5</span>
                                                </div>
                                                <div className="p-inputgroup">
                                                        <span className="p-inputgroup-addon dark:!bg-gray-400/60 dark:!text-gray-200">
                                                        ...
                                                        <InputText value={rating} onChange={(e) => setRating(e.target.value)} placeholder="Rating" className='dark:!text-gray-200 dark:!bg-gray-700/60'/>
                                                </div>
                                                ...
                                                <div className="pb-2"></div>
                                                <SelectButton onChange={(e) => setItemGenre(e.target.value)} optionLabel='nama' options={optionGenre} multiple value={itemGenre}/>
                                                
                                                <Divider className="!my-10"/>

                                                <span className="dark:text-gray-200 mt-3">Status</span>
                                                <div className="pb-2"></div>
                                                <SelectButton onChange={(e) => setItemStatus(e.target.value)} optionLabel='nama' options={optionStatus} multiple value={itemStatus}/>

                                                <Divider className="!my-10"/>
                                                
                                                <div className="group">
                                                        <span className="dark:text-gray-200 group-focus-within:hidden">Deskripsi</span>
                                                        <FloatLabel className='mt-3'>
                                                                <InputTextarea id="description" value={sinopsis} onChange={(e) => setSinopsis(e.target.value)} className='!w-full' rows={5} />
                                                                <label htmlFor="description">Deskripsi</label>
                                                        </FloatLabel>
                                                </div>

                                                <Button icon="pi pi-save" className='!mt-4' label='Simpan' onClick={() => handleSave()}/>
                                        </div>
                                </div>
                        </BlockUI>
                )
                                                ...
                                                {genreList.length !== 0 ? (
                                                        <>
                                                                {genreList.genre.map((item, index) => (
                                                                                ...
                                                                                ">
                                                                                        {item}
                                                                                        <Badge value="x" severity="danger" onClick={() => removeGenreListPreview(item)}/>
                                                                                </div>
                                                                ))}
                                                        </>
                                                ) : (
                                                        <>
                                                                <span className="text-gray-300 dark:text-gray-500">Data belum termuat</span>
                                                        </>
                                                )}
                                        </div>
                                        <Divider className='!mb-7'/>
                                        <div className="flex items-center gap-2 mt-4 w-fit">
                                                <input type="text" placeholder='Tambahkan Genre' value={addInputGenre} onChange={(e) => setAddInputGenre(e.target.value)} className='text-[.9rem] xs:text-base w-full py-2 px-3 border focus:outline-gray-300 rounded-md border-gray-300'/>
                                                <Button icon="pi pi-plus" className='!py-2 !px-3 !rounded-md' onClick={() => addGenreListPreview()}/>
                                        </div>
                                </div>    
                                <div className='mt-20'>
                                        <div className='flex gap-2 flex-wrap mt-2'>
                                                {statusList.length !== 0 ? (
                                                        <>
                                                                {statusList.status.map((item, index) => (
                                                                                ...
                                                                                ">
                                                                                        {item}
                                                                                        <Badge value="x" severity="danger" onClick={() => removeStatusListPreview(item)}/>
                                                                                </div>
                                                                ))}
                                                        </>
                                                ) : (
                                        ...
                                        <Divider className='!mb-7'/>
                                        <div className="flex items-center gap-2 mt-4 w-fit">
                                                <input type="text" placeholder='Tambahkan Status' value={addInputStatus} onChange={(e) => setAddInputStatus(e.target.value)} className='text-[.9rem] xs:text-base w-full py-2 px-3 border focus:outline-gray-300 rounded-md border-gray-300'/>
                                                <Button icon="pi pi-plus" className='!py-2 !px-3 !rounded-md' onClick={() => addStatusListPreview()} />
                                        </div>
                                        <Button icon="pi pi-save" label='simpan' className='!mt-4 !py-2 !px-3' onClick={() => sendSGtoFireStore(genreList.genre)}/>
                                </div>        
                        ...
                        <Splitter className='min-h-[85dvh] w-full dark:!bg-gray-800 !border-none'>
                                <SplitterPanel className="p-2" size={5} minSize={5}>
                                        <section className="dark:!text-gray-200 flex flex-col gap-10 items-center p-4 w-full">
                                                <div className={`item-dash ${menuActive === "home" ? "active" : ""}`} onClick={() => setMenuActive('home')}>
                                                        <i className="fas fa-house-user"></i>
                                                </div>
                                                <div className={`item-dash ${menuActive === "anime" ? "active" : ""}`} onClick={() => setMenuActive('anime')}>
